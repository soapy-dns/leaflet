// possibly the worst code ever where functions manipulate inputs and return other values
...
/*
 ...
 into 60 zones, each 6° of longitude in width.
 Zone 1 covers longitude 180° to 174° W;
 zone numbering increases eastward to zone 60 that covers
...
function MapLatLonToXY(phi, lambda, lambda0, xy = []) {
    ...
    /* Calculate easting (x) */
    const easting = N * Math.cos(phi) * l
        + (N / 6.0 * Math.pow(Math.cos(phi), 3.0) * l3coef * Math.pow(l, 3.0))
        + (N / 120.0 * Math.pow(Math.cos(phi), 5.0) * l5coef * Math.pow(l, 5.0))
        + (N / 5040.0 * Math.pow(Math.cos(phi), 7.0) * l7coef * Math.pow(l, 7.0));

    /* Calculate northing (y) */
    const northing = ArcLengthOfMeridian(phi)
 ...
 *
 * todo - why can't you get the zone from the lat long?
 * Inputs:
 ...
 *
 * WTF - this just returns what was put in!!!!!  UTMCentralMeridian doesn't change it
 */
